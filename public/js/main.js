...
const blackNotes = [
  0, 277.183, 311.127, 0, 369.994, 415.305, 466.164, 0, 554.365,
  622.254, 0, 739.989, 830.609, 932.328, 0, 1108.73, 1244.51, 0,
  1479.98, 1661.22, 0
]
...
const whiteNotesZone = ((canvasElement.height/2)-(whiteNoteHeight/2) + blackNoteHeight) / canvasElement.height
  ...
  if (results.multiHandLandmarks) {
    for (const landmarks of results.multiHandLandmarks) {
      ...
      for (let index = 4; index <= 20; index += 4) {
        if (landmarks[index].y > 1 / 3 && landmarks[index].y < 2 / 3 && isModalOpen === false) {
          if (landmarks[index - 1].y < landmarks[index].y) {
            if(landmarks[index].y > whiteNotesZone){
              ...
              blackKeys.forEach((k) => {
                let blackKeyStart = noteCoordsForCursor[k] - ((blackNoteWidth / canvasElement.width) / 2)
                let blackKeyStop = noteCoordsForCursor[k] + ((blackNoteWidth / canvasElement.width) / 2 )
                if(landmarks[index].x > blackKeyStart && landmarks[index].x < blackKeyStop) {
        ...
        if(landmarks[index - 1].y < landmarks[index].y && landmarks[index - 2].y > landmarks[index].y && !click) {
          let CursorPX = {'x': landmarks[index].x * canvasElement.width, 'y': landmarks[index].y * canvasElement.height}
          if(CursorPX) {
...
function play(n) {
  ...
  const now = context.currentTime
  galleryRecorded[n-1].forEach((i) => notesRecorded.play( i.frequency, now + i.time))
}
